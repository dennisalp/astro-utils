...
3. Write a 16-bit TIFF, carrying over the ICC colour profile of the
   first input file if one is present.
...
def copy_all_metadata(source_tiff_path, target_tiff_path):
    """
    Uses ExifTool via Python subprocess to copy all metadata natively.
    Requires ExifTool to be installed on your operating system.
    """
    command = [
        "exiftool",
        "-TagsFromFile", source_tiff_path,
        "-all:all",             # Copy every single tag exactly as is
        "-overwrite_original",  # Don't create an annoying '_original' backup file
    ...
    try:
        ...
        print(f"Metadata injected successfully into {target_tiff_path}")
        
    except FileNotFoundError:
        print("Error: ExifTool is not installed or not in your system's PATH.")
    except subprocess.CalledProcessError as e:
        print(f"ExifTool failed to write metadata:\n{e.stderr}")
...
def main():
    ...
    copy_all_metadata(files[ref_index], args.output)
    print("Done.")
