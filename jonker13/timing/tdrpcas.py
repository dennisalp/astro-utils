'''
2024-02-24, Dennis Alp, dalp@kth.se

Time-domain resampled partially coherent acceleration search.

Only this script runs FFTs. The other timing scripts I have should be
folding and computing test statistics per frequency.
...
def circular(x, tt):
    x['tot'] = x['nMp']*x['nMc']*x['nPb']*x['nTT'] / x['db']
    Mp = np.linspace(1.4, 2.5, x['nMp'])  # mass pulsar
    Mc = np.linspace(0.1, 1000, x['nMc'])  # mass companion
    Pb = np.linspace(200, 6000, x['nPb'])  # orbital period, period binary
        ...
        for T0 in TT:
            ...
            for mp in Mp:
                for mc in Mc:
                    ...
                    lc = get_lc(x, tr)
                    nu, po = search(x, lc)
                    out(x, nu, po, ['circular', mp, mc, pb, T0])
                    timing(x, x['start'], x['ctr'], x['ctr']/x['tot'])


def elliptic(x, tt):
    x['tot'] = x['nMp']*x['nMc']*x['nPb']*x['nTT']*x['nEc']*x['nWW'] / x['db']
    Mp = np.linspace(1.4, 2.5, x['nMp'])  # mass pulsar
    Mc = np.linspace(0.1, 1000, x['nMc'])  # mass companion
    Pb = np.linspace(200, 6000, x['nPb'])  # orbital period, period binary
                ...
                for mp in Mp:
                    for mc in Mc:
                        for ww in WW:
                            ...
                            lc = get_lc(x, tr)
                            nu, po = search(x, lc)
                            out(x, nu, po, ['elliptic', mp, mc, pb, T0, ec, ww])
                            timing(x, x['start'], x['ctr'], x['ctr']/x['tot'])
...
def scan(x, tt):
    ...
    # The following separation into two cases is arguably code repetition.
    if x['nEc'] == 1:
        circular(x, tt)
    else:
        elliptic(x, tt)
...
# This thing is only correct for `accel()`, but will run for binary searches with incorrect labeling
