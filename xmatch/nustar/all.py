'''
2020-06-16, Dennis Alp, dalp@kth.se

Scan all NuSTAR data. Search for transients, pulsars, and other stuff.

The version of the flight software that was installed at that time on
NuSTAR executed housekeeping operations at regular intervals (1 or 8
s), producing periodic dead time. Therefore, the raw power spectrum
and the CPDS of this observation are plagued by spurious features at 1
Hz and 0.125 Hz --- bachetti15
'''
...
def get_wcs(ff):
    dd = fits.open(ff)
    wcs = WCS()
    wcs.wcs.crpix = [dd[1].header['TCRPX38'], dd[1].header['TCRPX39']]
    wcs.wcs.cdelt = [dd[1].header['TCDLT38'], dd[1].header['TCDLT39']]
    wcs.wcs.crval = [dd[1].header['TCRVL38'], dd[1].header['TCRVL39']]
...
class Observation:
    def __init__(self, ii):
        ...
        self.xx = np.empty(0)
        self.yy = np.empty(0)
        self.tt_cam = {'A': np.empty(0), 'B': np.empty(0)}
        self.ee_cam = {'A': np.empty(0), 'B': np.empty(0)}
        self.xx_cam = {'A': np.empty(0), 'B': np.empty(0)}
        self.yy_cam = {'A': np.empty(0), 'B': np.empty(0)}
        self.keys = ['A', 'B']
        
        for ff in glob(cwd + 'nu' + self.oi + '*.evt'):
            self.cam = ff.split('/')[-1][13:14]
            ...
            self.ee_cam[self.cam] = np.concatenate((self.ee_cam[self.cam], dd['PI'][good]))
        ...
        if self.tt_cam['A'].size == 0 and self.tt_cam['B'].size == 0:
            print('WARNING No photons, skipping')
...
#
        ...
        for ii in range(2*len(self.keys)):
            ...
            ax = plt.subplot2grid((3,3), (ii//len(self.keys)+1, np.mod(ii,len(self.keys))), colspan=1, rowspan=1)
            tmp = self.img_cam[kk].T
            tmp = gaussian_filter(tmp, 4*(ii//len(self.keys)))
...
class Source:
    ...
    def ext_evt(self, obs):
        ...
        self.yy_cam = {}
        for kk in self.keys:
            src, bkg = get_src_bkg_idx(self, obs.tt_cam[kk], obs.xx_cam[kk], obs.yy_cam[kk])
            self.src_tt_cam[kk] = obs.tt_cam[kk][src]
            self.src_pi_cam[kk] = obs.ee_cam[kk][src]
            self.bkg_tt_cam[kk] = obs.tt_cam[kk][bkg]
            self.bkg_pi_cam[kk] = obs.ee_cam[kk][bkg]
            
            order = np.argsort(self.src_tt_cam[kk])
            self.src_tt_cam[kk] = self.src_tt_cam[kk][order]
            self.src_pi_cam[kk] = self.src_pi_cam[kk][order]
            order = np.argsort(self.bkg_tt_cam[kk])
            self.bkg_tt_cam[kk] = self.bkg_tt_cam[kk][order]
            self.bkg_pi_cam[kk] = self.bkg_pi_cam[kk][order]
    ...
    def mk_lc(self):
        ...
        self.sub_lc_cam = {}
        for kk in self.keys:
            self.src_lc_cam[kk], _ = np.histogram(self.src_tt_cam[kk], self.tlc)
            self.src_lc_cam[kk] = self.src_lc_cam[kk]/np.diff(self.tlc)
            self.bkg_lc_cam[kk], _ = np.histogram(self.bkg_tt_cam[kk], self.tlc)
            self.bkg_lc_cam[kk] = self.backscal*self.bkg_lc_cam[kk]/np.diff(self.tlc)
            self.sub_lc_cam[kk] = self.src_lc_cam[kk] - self.bkg_lc_cam[kk]
...
# CAM PDS            self.po_cam.append({})
# CAM PDS            for kk in self.keys:
# CAM PDS                self.po_cam[jj][kk] = fft_hlp(self.src_tt_cam[kk], dt, nn, ii)
...
# CAM PDS        ii = self.po_cam[0]['A'] > po_min
# CAM PDS        plt.loglog(self.nu[0][ii], self.po_cam[0]['A'][ii], 'o', label='FPMA, dt = {0:6.4f} s'.format(dT[0]))
# CAM PDS        ii = self.po_cam[0]['B'] > po_min
# CAM PDS        plt.loglog(self.nu[0][ii], self.po_cam[0]['B'][ii], 'o', label='FPMB, dt = {0:6.4f} s'.format(dT[0]))
        ii = self.po[1] > po_min
        plt.loglog(self.nu[1][ii], self.po[1][ii],          'o', label='Both, dt = {0:6.4f} s'.format(dT[1]))
# CAM PDS        ii = self.po_cam[1]['A'] > po_min
# CAM PDS        plt.loglog(self.nu[1][ii], self.po_cam[1]['A'][ii], 'o', label='FPMA, dt = {0:6.4f} s'.format(dT[1]))
# CAM PDS        ii = self.po_cam[1]['B'] > po_min
# CAM PDS        plt.loglog(self.nu[1][ii], self.po_cam[1]['B'][ii], 'o', label='FPMB, dt = {0:6.4f} s'.format(dT[1]))
        ax.legend()
...
# CAM PDS        po_max = np.amax((self.po[0].max(), self.po[1].max(), self.po_cam[0]['A'].max(), self.po_cam[0]['B'].max(), self.po_cam[1]['A'].max(), self.po_cam[1]['B'].max()))
...
def download_data(oi, ii):
    def dl_hlp(oi, ra, de):
        ...
        pp = oi[1:3]
        if cc == '0':
            print('Non-pointing data (e.g. IOC) or safe hold')
        elif cc == '1':
            print('Calibration observations, e.g. Crab nebula')
        elif cc == '2':
            print('Solar system objects (e.g. the Sun)')
        elif cc == '3':
            print('Galactic compact sources (stars, CVs, X-ray binaries, isolated neutron stars)')
        elif cc == '4':
            print('Non-ToO Supernovae, Supernova remnants, and Galactic diffuse emission')
        elif cc == '5':
            print('Normal galaxies')
        elif cc == '6':
            print('Active galaxies and quasars')
        elif cc == '7':
            print('Galaxy clusters and extragalactic diffuse objects')
        elif cc == '8':
            print('Proposed ToO\'s and Directors Discretionary Time')
        elif cc == '9':
            print('Non-proposal ToO\'s')

        subprocess.call(cmd + [wge.format(cc, pp, oi, 'A')])
        subprocess.call(cmd + [wge.format(cc, pp, oi, 'B')])
        subprocess.call(cmd + [orb.format(cc, pp, oi)])
        subprocess.call(['gunzip'] + ['nu{0:s}{1:s}01_cl.evt.gz'.format(oi, 'A')])
        subprocess.call(['gunzip'] + ['nu{0:s}{1:s}01_cl.evt.gz'.format(oi, 'B')])
        subprocess.call(['gunzip'] + ['nu' + oi + '_orb.fits.gz'])

        bca = ['barycorr', 'infile=' + cwd + 'nu{0:s}{1:s}01_cl.evt'.format(oi, 'A'), 'outfile=' + cwd + ffn.format(oi, 'A'),  'orbitfiles=' + cwd + 'nu' + oi + '_orb.fits', 'ra=' + str(ra), 'dec=' + str(de), 'refframe=ICRS', 'chatter=1']
        bcb = ['barycorr', 'infile=' + cwd + 'nu{0:s}{1:s}01_cl.evt'.format(oi, 'B'), 'outfile=' + cwd + ffn.format(oi, 'B'),  'orbitfiles=' + cwd + 'nu' + oi + '_orb.fits', 'ra=' + str(ra), 'dec=' + str(de), 'refframe=ICRS', 'chatter=1']
        subprocess.call(bca)
        subprocess.call(bcb)
        subprocess.call(['rm'] + glob('*cl.evt') + glob('*orb*'), stderr=subprocess.DEVNULL)
        return True

    ra = coo[ii].ra.degree
    de = coo[ii].dec.degree
    
    if os.path.isfile(ffn.format(oi, 'A')):
...
################################################################
# Just read the data
obs, target, coo, epoch, exp = read_nus('/Users/silver/box/phd/pro/obs/met/dat/nus_log.txt')
cwd = '/Volumes/pow/dat/nus/cat/'
ouf = '/Users/silver/out/fig/'
oud = '/Users/silver/out/dat/'
...
cmd = ['wget', '-nv']
wge = 'https://heasarc.gsfc.nasa.gov/FTP/nustar/data/obs/{1:s}/{0:s}/{2:s}/event_cl/nu{2:s}{3:s}01_cl.evt.gz' # 01 (SCIENCE): normal observing scientific mode;
orb = 'https://heasarc.gsfc.nasa.gov/FTP/nustar/data/obs/{1:s}/{0:s}/{2:s}/auxil/nu{2:s}_orb.fits.gz'
ffn = 'nu{0:s}{1:s}01_bc.evt'
...
sbin = 20 # pixel size of 2.45810736 arcsec
hips_width = 256
hips_height = 256
ene_low = 3.0 # keV
ene_high = 79.0 # keV
...
stop = int(sys.argv[2])
for ii, oi in enumerate(obs[start:stop]): # 2385/2587
    # Housekeeping
    ii = ii + start
